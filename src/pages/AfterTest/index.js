...
const LabelName = props => {
  return (
    <View style={styles.label}>
      <Icon name={props.iconName} size={32} color={
    '#000'} />
...
const AfterTest = props => {
  ...
  useEffect(() => {
    ...
    if (kid.sex === 'M') {
      setLengthList(
          require('../../assets/growth/AlturaMasculino.json')
              .filter(
                  obj => obj.mes === month,
                  ),
      );
      setWeightList(
          require('../../assets/growth/PesoMasculino.json')
              .filter(
                  obj => obj.mes === month,
                  ),
      );
      ...
      setLengthList(
          require('../../assets/growth/AlturaFeminino.json')
              .filter(
                  obj => obj.mes === month,
                  ),
      );
      setWeightList(
          require('../../assets/growth/PesoFeminino.json')
              .filter(
                  obj => obj.mes === month,
                  ),
      );
...
function getWeightResultInfo() {
  ...
  return <LabelName iconName = {iconName} text = {
    text
  } />;
  }
  ...
  if (ok === true) {
    NetInfo.fetch().then(state => {
      if (state.isInternetReachable) {
        api.post('/registerKid', body, {timeout : 10000})
            .then(res => {
...
if (name === '' || birthAge === '' || weight === '' || length === '' ||
    weightList === '' || lengthList === '') {
  return null;
}
      ...
      <ScrollView style={styles.container}>
        <View style={styles.info}>
          ...
          <View style={styles.box}>
            ...
            <LabelName iconName={'today'} text={
  birthAge} />
            <LabelName iconName={'linear-scale'} text={`${weight}kg`} />
            <LabelName iconName={'straighten'} text={
  `${length}cm`} />
          </View>
        </View>
